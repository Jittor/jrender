...
def main():
    ...
    parser.add_argument('-i', '--filename-input', type=str, 
        default=os.path.join(data_dir, 'scene_bunny'))
    parser.add_argument('-o', '--output-dir', type=str,  
        default=os.path.join(data_dir, 'results/output_render'))
    args = parser.parse_args()
    
    render = Render(image_size=2048,camera_mode="look",eye=[0,0.8,4.1],camera_direction=[-0.05,-0.1,-1],near=0.5,viewing_angle=30)
    ...
    light1 = Light(position=[2.4,4,2],direction=[-2.4,-4, -2],intensity=1.0,type="point",shadow=False)
    light3 = Light(position=[-3,4,0.3],direction=[3,-4, 0],intensity=1.0,type="point",shadow=False)
    ...
    writer = imageio.get_writer(os.path.join(args.output_dir, 'bunny_with_sssr.jpg'))
