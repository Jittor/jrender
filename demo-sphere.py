...
def main():
    ...
    parser.add_argument('-i', '--filename-input', type=str, 
        default=os.path.join(data_dir, 'scene_stool'))
    parser.add_argument('-o', '--output-dir', type=str,  
        default=os.path.join(data_dir, 'results/output_render'))
    args = parser.parse_args()
    #TODO:light area 
    render = Render(image_size=2048,camera_mode="look",eye=[-3.5,2.5,2.5],camera_direction=[1.2,-1.2,-1],near=0.5)
    ...
    scene.append_light([light3,light2])
    scene.set_rescaling(1,1)
    scene.set_GenerateNormal(1,"from_obj")
    scene.set_roughness([0,1],0.4)
    ...
    writer = imageio.get_writer(os.path.join(args.output_dir, 'stool_soft_shadow.jpg'))
