...
def create_texture_image(vertices_all, textures, image, eps):
    return jt.code(image.shape, image.dtype, [vertices_all, textures],
    cuda_header='''
#include <cmath>
...
__global__ void create_texture_image_cuda_kernel(
    ...
    const auto num_faces = textures_shape0;
    const auto texture_res_in = int(sqrt(textures_shape1));
    const auto tile_width = int(sqrt(num_faces - 1)) + 1;
    const auto texture_res_out = image_shape1 / tile_width;

    const int threads = 1024;
    const int image_size = image->size;
    const dim3 blocks ((image_size / 3 - 1) / threads + 1);

    create_texture_image_cuda_kernel<float32><<<blocks, threads>>>(
        faces_p,
        textures_p,
        image_p,
        image_size,
