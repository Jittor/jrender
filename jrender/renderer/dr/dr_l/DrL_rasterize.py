...
import numpy as np
from .cuda import DrL_rasterize as soft_rasterize_cuda

class DrL_RasterizeFunction(Function):
    ...
    def execute(self, face_vertices, textures):
        ...
        points = jt.zeros((self.batch_size, 2, self.image_size, self.image_size))
        msk = jt.zeros((self.batch_size, 1, self.image_size, self.image_size))

        # TODO: soft_colors do not init in cuda
        faces_info, aggrs_info, soft_colors, faces_id_buffer, points, msk = \
            soft_rasterize_cuda.forward_DrL_rasterize(face_vertices, textures,
        ...
        self.save_vars = face_vertices, textures, soft_colors, points, faces_info, aggrs_info, faces_id_buffer
        return soft_colors, points, msk

    def grad(self, grad_soft_colors, grad_points, grad_msk):

        face_vertices, textures, soft_colors, points, faces_info, aggrs_info, faces_id_buffer = self.save_vars
        ...
        grad_faces, grad_textures = \
            soft_rasterize_cuda.backward_DrL_rasterize(face_vertices, textures, soft_colors, points, 
...
def DrL_rasterize(face_vertices, textures, image_size=256, 
                   background_color=[0, 0, 0], near=1, far=100, 
                   fill_back=True, eps=1e-3,
                   sigma_val=1e-5, dist_func='euclidean', dist_eps=1e-4,
                   gamma_val=1e-4, aggr_func_rgb='softmax', aggr_func_alpha='prod',
                   texture_type='surface', bin_size=0, max_elems_per_bin=0, max_faces_per_pixel_for_grad=16):
    return DrL_RasterizeFunction(image_size, 
