...
def forward_DrL_rasterize(face_vertices, textures,
    ...
    return jt.code([faces_info.shape, aggrs_info.shape, soft_colors.shape, faces_id_buffer.shape, points.shape, msk.shape], [faces_info.dtype, aggrs_info.dtype, soft_colors.dtype, faces_id_buffer.dtype, points.dtype, msk.dtype],
...
namespace{
    ...
    __global__ void forward_soft_rasterize_cuda_kernel(
        ...
        points[(bn * 2 + 0) * (is * is) + pn] = xp;
        points[(bn * 2 + 1) * (is * is) + pn] = yp;
...
///////////////////////////////////////////////////
            ...
            if (zp < depth_min && check_pixel_inside(w) && (double_side || check_face_frontside(face))){
                points[(bn * 2 + 0) * (is * is) + pn] = w_clip[0]*face[0] + w_clip[1]*face[3] + w_clip[2]*face[6];
                points[(bn * 2 + 1) * (is * is) + pn] = w_clip[0]*face[1] + w_clip[1]*face[4] + w_clip[2]*face[7];
                msk[(bn * 1 + 0) * (is * is) + pn] = 1.0;
            }
            ...
            if (func_id_rgb == 0) { // Hard assign
                if (zp < depth_min && check_pixel_inside(w) && (double_side || check_face_frontside(face))) {
                    ...
                    face_index_min = fn;
                    for (int k = 0; k < 3; k++) {
                        soft_color[k] = forward_sample_texture(texture, w_clip, texture_res, k, texture_sample_type, face, zp);
                    }
                }
            } else
    ...
    cudaMemsetAsync(out4_p, 0, out4->size);
    cudaMemsetAsync(out5_p, 0, out5->size);
...
def backward_DrL_rasterize(faces, textures, soft_colors, points, 
    faces_info, aggrs_info, faces_id_buffer,
    grad_faces, grad_textures, grad_soft_colors, grad_points, 
    image_size, near, far, eps,
    sigma_val, func_id_dist, dist_eps,
    gamma_val, func_id_rgb, func_id_alpha,
    texture_sample_type, double_side):
    return jt.code([grad_faces.shape, grad_textures.shape], [grad_faces.dtype, grad_textures.dtype], [faces, textures, soft_colors, points,
    faces_info, aggrs_info, grad_soft_colors, faces_id_buffer, grad_points],
    cuda_header='''
#include <cuda.h>
#include <cuda_runtime.h>
...
__global__ void backward_soft_rasterize_cuda_kernel(
    ...
    for (int m = 0; m < mf; m++) {
        ...
        const scalar_t grad_px = grad_points[(bn * 2 + 0) * (is * is) + pn];
        const scalar_t grad_py = grad_points[(bn * 2 + 1) * (is * is) + pn];

        grad_v[0][0] += grad_px*w[0] / face[2] / face[2];
        grad_v[0][1] += grad_py*w[0] / face[2] / face[2];
        grad_v[1][0] += grad_px*w[1] / face[5] / face[5];
        grad_v[1][1] += grad_py*w[1] / face[5] / face[5];
        grad_v[2][0] += grad_px*w[2] / face[8] / face[8];
        grad_v[2][1] += grad_py*w[2] / face[8] / face[8];
    ...
    @alias(points, in3)
    @alias(faces_info, in4)
    @alias(aggrs_info, in5)
    @alias(grad_soft_colors, in6)
    @alias(faces_id_buffer,in7)
    @alias(grad_points, in8)
